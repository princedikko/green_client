...
export default function Discripancies({ breadcrumbs }) {
  ...
  async function apiPostDiscripancy() {
    try {
      setLoading(true);
      const response = await axios.post(
        `${process.env.REACT_APP_SERVER_SCRIPT_HOST}/client/691a663dc9f64e6b9b8be48e/stock_management/stock-discripancy`,
        payload,
      );
  ...
  const apiGetDiscripancies = async () => {
    setLoading(true);
    await axios
      .get(
      ...
      .then((response) => {
        discripancyData = response.data.data;
        console.log("discripancyData: ", response);
        if (response.data.status === 201) {
          setLoading(false);
          enqueueSnackbar(`${response.data.message}`, {
            variant: "success",
  ...
  function Progress() {
    ...
    function TableView({ currentRows }) {
      return (
        <div className="prog">
          <button className="btnTemporary" onClick={() => apiPostDiscripancy()}>
            Post Return
          </button>
          <table className="fx-cl spacem">
            <thead className="fx-cl spacem">
