...
export default function SystemAdminLogin() {
  ...
  const state = useSelector((state) => state.systemAdminFunction);
  ...
  // FETCH DATA FM DB
  const [systemAdminLoginData, setSystemAdminLoginData] = useReducer(
    (reqest, response) => {
      return { ...reqest, ...response };
    },
    { user_name: "", systemAdmin_password: "" }
  );

  const executeLogin = async () => {
    const data = {
      user_name: systemAdminLoginData.user_name,
      systemAdmin_password: systemAdminLoginData.systemAdmin_password,
    };
    setLoaiding(true);
    await axios
      .post(
        `${process.env.REACT_APP_SERVER_SCRIPT_HOST}/system-administration/login`,
        data
      )
      .then((response) => {
        systemAdminData = response.data.info;
        dispatch(DispatchSystemAdminData());
        if (response?.data.status === 201) {
          ...
          redirect("/website-management/system-adming/account");
  ...
  const DispatchSystemAdminData = () => async (dispatch) => {
    try {
      dispatch(Action.startSystemAdminAction({ systemAdminData }));
    } catch (error) {
      console.log(error);
    }
  };
        ...
        <div className="management-login-container rwdG2">
            ...
            <div className="management-login-form fx-cl space3">
              <div className="fx-cl spacem">
                ...
                <div className="fx-ac space1">
                  ...
                  <input
                    ...
                    value={systemAdminLoginData.user_name}
                    onChange={(event) =>
                      setSystemAdminLoginData({ user_name: event.target.value })
                    }
                    type="text"
              ...
              <div className="fx-cl spacem">
                ...
                <div className="fx-ac space1">
                  ...
                  <input
                    name="systemAdmin_password"
                    onKeyPress={handleKeyPress}
                    value={systemAdminLoginData.systemAdmin_password}
                    onChange={(event) =>
                      setSystemAdminLoginData({
                        systemAdmin_password: event.target.value,
                      })
                    }
              ...
              {/* <button ><Link to="/systemAdmin_profile" style={{color: "white"}}>Login</Link></button> */}
              ...
              <button
                ...
                onClick={() =>
                  redirect("/website-management/system-adming/employements")
                }
              >
                Employment
              </button>
          ...
          <div className="management-login-disc fx-cl fx-ac space4 rwdHide">
            <div className="fx-cl space2">
              <h4>Welcome back systemAdmin!, Let's get to Work</h4>
              <p style={{ fontSize: "1.6rem" }}>
                This Login is specifically for the systemAdministrative Staff of
