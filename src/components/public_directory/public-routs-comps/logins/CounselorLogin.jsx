...
export default function CounselorLogin() {
  ...
  const state = useSelector((state) => state.counselorFunction);
  ...
  // FETCH DATA FM DB
  const [counselorLoginData, setCounselorLoginData] = useReducer(
    (reqest, response) => {
  ...
  const executeLogin = async () => {
    const data = {
      user_name: counselorLoginData.user_name,
      counselor_password: counselorLoginData.counselor_password,
    };
    setLoaiding(true);
    await axios
      .post(
      ...
      .then((response) => {
        ...
        dispatch(DispatchCounselorData());
        if (response?.data.status === 201) {
          ...
          redirect("/support/giudance-and-counselling/account");
  ...
  const DispatchCounselorData = () => async (dispatch) => {
    try {
      dispatch(Action.startCounselorAction({ counselorData }));
    } catch (error) {
      console.log(error);
    }
  };
        ...
        <div className="management-login-container rwdG2">
            ...
            <div className="management-login-form fx-cl space3">
              <div className="fx-cl spacem">
                ...
                <div className="fx-ac space1">
                  ...
                  <input
                    ...
                    value={counselorLoginData.user_name}
                    onChange={(event) =>
                      setCounselorLoginData({ user_name: event.target.value })
                    }
                    type="text"
              ...
              <div className="fx-cl spacem">
                ...
                <div className="fx-ac space1">
                  ...
                  <input
                    name="counselor_password"
                    onKeyPress={handleKeyPress}
                    value={counselorLoginData.counselor_password}
                    onChange={(event) =>
                      setCounselorLoginData({
                        counselor_password: event.target.value,
                      })
                    }
              ...
              {/* <button ><Link to="/counselor_profile" style={{color: "white"}}>Login</Link></button> */}
              ...
              <button
                ...
                onClick={() => redirect("/counselor_registration")}
              >
                Employment
              </button>
            </div>
          ...
          <div className="management-login-disc fx-cl fx-ac space4 rwdHide">
            <div className="fx-cl space2">
              <h4>Welcome back counselor!, Let's get to Work</h4>
              <p style={{ fontSize: "1.6rem" }}>
                This Login is specifically for the counseloristrative Staff of
