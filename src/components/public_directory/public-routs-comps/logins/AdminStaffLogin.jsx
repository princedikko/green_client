...
export default function AdminStaffLogin() {
  ...
  const state = useSelector((state) => state.adminFunction);
  ...
  // FETCH DATA FM DB
  const [adminLoginData, setAdminLoginData] = useReducer(
    (reqest, response) => {
  ...
  const executeLogin = async () => {
    const data = {
      user_name: adminLoginData.user_name,
      admin_password: adminLoginData.admin_password,
    };
    setLoaiding(true);
    await axios
      .post(`${process.env.REACT_APP_SERVER_SCRIPT_HOST}/admin_login`, data)
      .then((response) => {
        adminData = response.data.info;
        dispatch(DispatchAdminData());
        if (response?.data.status === 201) {
          ...
          redirect("/administration/admin-account");
  ...
  const DispatchAdminData = () => async (dispatch) => {
    try {
      dispatch(Action.startAdminAction({ adminData }));
    } catch (error) {
      console.log(error);
    }
  };
        ...
        <div className="management-login-container rwdG2">
          <div className="management-login-form-cont fx-cl space2">
            ...
            <div className="management-login-form fx-cl space3">
              <div className="fx-cl spacem">
                ...
                <div className="fx-ac space1">
                  ...
                  <input
                    ...
                    value={adminLoginData.user_name}
                    onChange={(event) =>
                      setAdminLoginData({ user_name: event.target.value })
                    }
                    type="text"
              ...
              <div className="fx-cl spacem">
                ...
                <div className="fx-ac space1">
                  ...
                  <input
                    name="admin_password"
                    onKeyPress={handleKeyPress}
                    value={adminLoginData.admin_password}
                    onChange={(event) =>
                      setAdminLoginData({ admin_password: event.target.value })
                    }
              ...
              {/* <button ><Link to="/admin_profile" style={{color: "white"}}>Login</Link></button> */}
              ...
              <button
                ...
                onClick={() => redirect("/admin_registration")}
              >
                Employment
              </button>
            </div>
          ...
          <div className="management-login-disc fx-cl fx-ac space4 rwdHide">
            <div className="fx-cl space2">
              <h4>Welcome back Admin!, Let's get to Work</h4>
              <p style={{ fontSize: "1.6rem" }}>
                This Login is specifically for the Administrative Staff of Manga
