...
import * as Action from "../../../../store/redux/security_reducer.js";
...
export default function SecurityLogin() {
  ...
  const state = useSelector((state) => state.securityFunction);
  ...
  // FETCH DATA FM DB
  const [securityLoginData, setSecurityLoginData] = useReducer(
    (reqest, response) => {
      return { ...reqest, ...response };
    },
    { user_name: "", security_password: "" }
  );

  const executeLogin = async () => {
    const data = {
      user_name: securityLoginData.user_name,
      security_password: securityLoginData.security_password,
    };
    setLoaiding(true);
    await axios
      .post(
        `${process.env.REACT_APP_SERVER_SCRIPT_HOST}/security-routes/staff/login`,
        data
      )
      .then((response) => {
        securityData = response.data.info;
        dispatch(DispatchSecurityData());
        if (response?.data.status === 201) {
          ...
          redirect("/institution-security/account");
  ...
  const DispatchSecurityData = () => async (dispatch) => {
    try {
      dispatch(Action.startSecurityAction({ securityData }));
    } catch (error) {
      console.log(error);
    }
  };
        ...
        <div className="management-login-container rwdG2">
          <div className="management-login-form-cont fx-cl space2">
            ...
            <div className="management-login-form fx-cl space3">
              <div className="fx-cl spacem">
                ...
                <div className="fx-ac space1">
                  ...
                  <input
                    ...
                    value={securityLoginData.user_name}
                    onChange={(event) =>
                      setSecurityLoginData({ user_name: event.target.value })
                    }
                    type="text"
              ...
              <div className="fx-cl spacem">
                ...
                <div className="fx-ac space1">
                  ...
                  <input
                    ...
                    onKeyPress={handleKeyPress}
                    value={securityLoginData.security_password}
                    onChange={(event) =>
                      setSecurityLoginData({
                        security_password: event.target.value,
                      })
                    }
              ...
              {/* <button ><Link to="/security_profile" style={{color: "white"}}>Login</Link></button> */}
              ...
              <button
                ...
                onClick={() => redirect("/security_registration")}
              >
                Employment
              </button>
            </div>
          ...
          <div className="management-login-disc fx-cl fx-ac space4 rwdHide">
            <div className="fx-cl space2">
              <h4>Welcome back security!, Let's get to Work</h4>
              <p style={{ fontSize: "1.6rem" }}>
                This Login is specifically for the securityistrative Staff of
