      ...
      {dataFetch?.map((response, index) => {
        return (
          <figure className="couresOfStudyCard fx-cl space1">
            ...
            >
              <em>{response?.hostelName}</em>
            </div>

            <div className="fx-cl ">
              <span>Room No.: {response.roomNumber}</span>
              <span>Statue: {response.status}</span>
              <span>Session: {response.session}</span>
...
export default function Hostels() {
  ...
  async function findApplicationNo() {
    setLoading(true);
    await axios
      .get(`${process.env.REACT_APP_SERVER_SCRIPT_HOST}/admin/fetch_hostels`)
      .then((response) => {
            ...
            <header className="fx-ac fx-jb space4">
              <div className="fx-ac space3">
                <input
                  ...
                  placeholder="Enter Hostel Build Name..."
                />
